...
void print_vertex(vertex* vertex_ptr) {
    ...
    for (int i = 0; i < vertex_ptr->edge_count; i++) {
        vertex* neighbor = (vertex_ptr->edges[i]->source == vertex_ptr) ? 
                          vertex_ptr->edges[i]->target : 
                          vertex_ptr->edges[i]->source;
...
void bfs(graph* graph_ptr, vertex* start_vertex) {
    printf ("%d\n", start_vertex-> data);
    for (int i = 0; i < start_vertex->edge_count; i++){
        
        edge* current_edge = start_vertex->edges[i];
        
        if (current_edge->source != start_vertex) {
            printf("%d\n", current_edge->source->data);
        } else {
            printf("%d\n", current_edge->target->data);
        }
